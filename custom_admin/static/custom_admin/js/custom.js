...
$(document).ready(function() {
    ...
    var accordion_menu = function() {
        ...
        // Hide all sub-menus and ensure no 'open' class
        $('.sub-menu').hide();
        $('.accordion-menu-item').removeClass('open');
        ...
        $('.accordion-menu').on('click', 'a', function(e) {
            ...
            // Prevent default behavior for accordion toggles
            if (parent_list_el.hasClass('has-submenu') && $(this).attr('href') === '#') {
                e.preventDefault();
            }
            ...
            if((sub_menu.length)&&(!body.hasClass('page-sidebar-collapsed'))) {
                // Check if submenu is actually visible, not just if parent has 'open' class
                if(!sub_menu.is(':visible')) {
                    // Remove open class from all others first
                    $('.accordion-menu-item').removeClass('open');
                    $('.sub-menu').slideUp(submenu_animation_speed);
                    // Then show this one
                    show_sub_menu();
                } else {
                    hide_sub_menu();
    ...
    //Toggle Mode (disabled - dark mode removed)
    ...
    //Active Left Sidebar Script 
     ...
     var closestParent1 = activeLi.length > 0 && activeLi.get(0) && activeLi.get(0).parentNode ? activeLi.get(0).parentNode.closest('li') : null;
